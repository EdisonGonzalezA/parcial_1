...
class Clientes
{
    ...
    public function uno($cliente_id) //select * from cliente where id = $idCliente
    {
        $con = new ClaseConectar();
        $con = $con->ProcedimientoParaConectar();
    ...
    public function actualizar($cliente_id, $nombre, $apellido, $email, $telefono, $direccion) //update cliente set $datos where id = $idCliente
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
            $cadena = "UPDATE `clientes` SET `cliente_id`='$cliente_id', `nombre`='$nombre',`apellido`='$apellido',`email`='$email',`telefono`='$telefono',`direccion`='$direccion' WHERE `cliente_id`= $cliente_id";
            if (mysqli_query($con, $cadena)) {
                return $cliente_id;
            } else {
                return $con->error;
